...
FIX: os.path.relpath -> os.path.abspath to support cross-drive exports (e.g. D:/).
"""
...
class CronbachAlphaCalculator:
    ...
    def compute(data):
        ...
        se, ci_lo, ci_hi = (
            (None, None, None) if n <= 3
            else CronbachAlphaCalculator.compute_alpha_ci(avg_r, k, n)
        )
        perfect = [
            (clean.columns[i], clean.columns[j])
            for i in range(k)
        ...
        return dict(
            alpha=alpha, avg_interitem_corr=avg_r, std_error=se,
    ...
    def generate(results, description, filename, title=None, subtitle=None, byline=None):
        ...
        if results.get('perfect_correlations'):
            ...
            els.append(Paragraph(
                f"<i>Note.</i> Variables {' and '.join(pairs)} correlated perfectly.",
                its))
        ...
        # FIX: use abspath instead of relpath to avoid cross-drive errors on Windows
        els.append(Paragraph(
            f"File: {os.path.abspath(filename)}<br/>Generated: {ts2}", fs))
        doc.build(els)
...
class DataTableFrame(ctk.CTkScrollableFrame):
    ...
    def display_data(self, df, max_rows=None):
        ...
        ctk.CTkLabel(self, text=f"📊 {len(df)} rows × {len(df.columns)} cols",
                     ...
                     text_color=ACCENT).pack(pady=(6, 8))
        ...
        for idx, row in preview_df.iterrows():
            ...
            for val in row:
                ctk.CTkLabel(rf,
                             text=(str(val)[:12] if pd.notna(val) else ""),
...
class LikertWindow(ctk.CTkToplevel):
    ...
    def _build(self):
        ...
        ctk.CTkLabel(cfg_row, text="Number of Items:", font=FONT_BODY,
                     text_color=TEXT_SEC).pack(side="left", padx=(0, 8))
        self.num_entry = styled_entry(cfg_row, placeholder="e.g. 10",
                                      width=90, height=32)
        ...
        ctk.CTkButton(btn_row, text="✓  Generate Dataset", command=self._generate,
                      ...
                      font=FONT_BTN, height=40, corner_radius=8).pack(
            side="left", padx=(0, 8))
    ...
    def _generate(self):
        if not self.entries:
            messagebox.showwarning("No Fields", "Create fields first.", parent=self)
            return
        try:
            items_data = {}
            for nm in self.entries:
                ...
                if not ok:
                    messagebox.showerror("Error", f"{nm}: {err}", parent=self)
                    return
                items_data[nm] = fd
...
class CronbachAlphaApp(ctk.CTk):
    ...
    def _build_ui(self):
        ...
        ctk.CTkLabel(hdr, text="JASP-Compatible Formula",
                     font=("Segoe UI", 11), text_color=TEXT_SEC).pack(
            side="right", padx=24)
        ...
        self.stat_label = ctk.CTkLabel(bar, text="",
                                        font=("Segoe UI", 11, "bold"),
                                        text_color=ACCENT)
    ...
    def import_data(self):
        ...
        if not fp:
            return
        try:
            self.df = (pd.read_csv(fp) if fp.endswith(".csv")
    ...
    def compute_alpha(self):
        if self.df is None:
            messagebox.showwarning("No Data", "Import or generate data first.")
            return
        try:
            ...
            txt += (f"{'═' * 40}\n"
                    ...
                    f"{'═' * 40}\n")
            txt += "\n".join(
                f"  {i + 1}. {nm}" for i, nm in enumerate(r['item_names']))
    ...
    def export_pdf(self):
        if self.results is None:
            messagebox.showwarning("No Results", "Compute alpha first.")
            return
    ...
    def save_part(self):
        ...
        # Advance only the letter, keeping the same Part number
    ...
    def _export_all_docx(self):
        if not self.saved_parts:
            messagebox.showwarning("Empty Session",
                "No parts saved yet. Save at least one part first.")
            return
        fp = filedialog.asksaveasfilename(
            defaultextension=".docx",
            filetypes=[("Word Document", "*.docx")],
            initialfile=(f"CronbachAlpha_Session_"
    ...
    def apply_settings(self):
        ...
        self.sidebar.compute_btn.configure(
    ...
    def clear_all(self):
        ...
        # Preserve Part number when clearing — only advance the letter
        current    = self._current_title_entry()
        next_label = _next_part_label(
            [p["label"] for p in self.saved_parts],
            current_label=current
